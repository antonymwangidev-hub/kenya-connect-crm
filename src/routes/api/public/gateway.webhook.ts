...
// Nexus WhatsApp Gateway receiving endpoint.
// Public URL: https://<app>/api/public/gateway/webhook
// Registered automatically when the gateway is connected in Settings.
...
/** Find which business this event belongs to by matching the signature against stored secrets. */
async function resolveBusiness(rawBody: string, signature: string | null) {
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (payload?.event === "webhook.test") {
          await logEvent(null, { event: "webhook.test" });
          return new Response("ok", { status: 200 });
        }

        const signature = request.headers.get("X-Gateway-Signature");
        const businessId = await resolveBusiness(rawBody, signature);
        if (!businessId) {
          await logEvent(null, { event: payload?.event ?? "unknown" }, "Invalid X-Gateway-Signature");
